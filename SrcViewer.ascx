<%@ Control Language="C#" ClassName="SrcViewer" %>
...
<script runat="server" language="C#">
            ...
            if (filename != null)
            {
                ...
                else
                {
                    ...
                    while ((sourceLine = sr.ReadLine()) != null)
                    {
                        ...
                        if (IsNonSrcContentTagStart(sourceLine))
                        {
                            isInNonSrcContent = true;
                        }                        
                        if (IsContentPlaceHolderEnd(sourceLine))
                        {
                            isInNonSrcContent = false;
                        }
                        else if (!isInNonSrcContent)
                        {
    ...
    bool IsNonSrcContentTagStart(String source)
    {
        return (source.Contains("<asp:Content") && source.Contains("ContentPlaceHolderID") && !source.Contains("RunningCodeContentPlaceHolder"));
    }

    bool IsContentPlaceHolderEnd(String source)
    {
        return Regex.IsMatch(source, "</asp:Content.*>");
    }
