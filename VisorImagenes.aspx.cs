    ...
    protected void Page_Load(object sender, EventArgs e)
    {
      ...
      if (user == null)
      {
        ...
        try
        {
            ...
            if (!String.IsNullOrEmpty(Grupo) && !String.IsNullOrEmpty(nrodoc))
            {
                ...
                string StrConexion = ConfigurationManager.ConnectionStrings["ConnStrSQLServer"].ConnectionString;
                SqlConnection ObjConexion = new SqlConnection(StrConexion);
                SqlCommand ObjComando = new SqlCommand();
                ObjComando.Connection = ObjConexion;
                ObjComando.CommandType = CommandType.StoredProcedure;
                ObjComando.Parameters.Add("DocumentoCodigo", SqlDbType.VarChar).Value = nrodoc;
                ObjComando.Parameters.Add("GrupoCodigo", SqlDbType.VarChar).Value = Grupo;
                ObjConexion.Open();
                ObjComando.CommandText = "WS_Read_RutaImagenByTramiteV2";
                SqlDataReader objLector = ObjComando.ExecuteReader(CommandBehavior.CloseConnection);
                if (null != objLector) 
                {
                    if(!objLector.HasRows)
                    {
                        LblDocumentoNro.Text = "No existen imagenes para este radicado " + HFNroDoc.Value;
                        LDocumentoFolio.Text = "";
                    }
                    else{
                        int tmFolio = 1;
                        ...
                        {
                            string a;
                            a = objLector["GrupoCodigo"].ToString();
                            a = objLector["ExpedienteCodigo"].ToString();
                            NombreArch = objLector["Ruta"].ToString().Substring(objLector["Ruta"].ToString().LastIndexOf("\\") + 1);
                            RutaArchivo = objLector["Ruta"].ToString().Substring(0, objLector["Ruta"].ToString().LastIndexOf("\\")) + "\\";


                            this.LblDocumentoNro.Text = objLector["Grupo"].ToString() +": "+ objLector["DocumentoNumero"].ToString();
                            this.LDocumentoFolio.Text = "Folio: " + tmFolio.ToString();
                            HtmlTable Tabla = new HtmlTable();
                            ShowThumbanails(Tabla, j, NombreArch, RutaArchivo, tmFolio.ToString(), objLector["DocumentoNumero"].ToString());
                            this.PlaceHolder1.Controls.Add(Tabla);

                            if (tmFolio == Convert.ToInt32(Request["ImagenFolio"]))
                            {
                                this.HFPath.Value = RutaArchivo;
            ...
            if (!this.IsPostBack)
            {
                ...
                if (TipoArchivo.ToLower() == "gif" || TipoArchivo.ToLower() == "jpg" || TipoArchivo.ToLower() == "png" || TipoArchivo.ToLower() == "bmp" || TipoArchivo.ToLower() == "tif")
                {
...
//////////////////////////////////////////////////////////
                ...
                if (TipoArchivo.ToLower() == "txt" || TipoArchivo.ToLower() == "pdf" || TipoArchivo.ToLower() == "doc" || TipoArchivo.ToLower() == "docx" || TipoArchivo.ToLower() == "xls" || TipoArchivo.ToLower() == "xlsx" || TipoArchivo.ToLower() == "ppt" || TipoArchivo.ToLower() == "pptx")
                {
    ...
    public void ShowThumbanails(HtmlTable Table, int i, String ImagenNombre, String Path, String Folio, String NumeroDocumento)
    {
        try
        {
            ...
            if (words[Ind].ToLower() == "doc" || words[Ind].ToLower() == "docx")
            {
                Hpl.ImageUrl = "~/AlfaNetImagen/iconos/icono_doc.gif";
                //Hpl.Target = "_Blank";
                Hpl.NavigateUrl = "VisorImagenes.aspx?DocumentoCodigo=" + NumeroDocumento + "&GrupoPadreCodigo=" + GrupoPadreCodigo + "&GrupoCodigo=" + GrupoCodigo + "&ImagenFolio=" + Folio;
                //Hpl.NavigateUrl = VirtualPath + ImagenNombre;
                //Hpl.NavigateUrl = Path + ImagenNombre;

            }
            else if (words[Ind].ToLower() == "xls" || words[Ind].ToLower() == "xlsx")
            {
            ...
            else if (words[Ind].ToLower() == "tif")
            {
                Hpl.ImageUrl = "~/AlfaNetImagen/iconos/icono_tif.jpg";
                Hpl.NavigateUrl = "VisorImagenes.aspx?DocumentoCodigo=" + NumeroDocumento + "&GrupoPadreCodigo=" + GrupoPadreCodigo + "&GrupoCodigo=" + GrupoCodigo + "&ImagenFolio=" + Folio;
            }
            else if (words[Ind].ToLower() == "jpg")
            {
                Hpl.ImageUrl = "~/AlfaNetImagen/iconos/icono_tif.jpg";
                Hpl.NavigateUrl = "VisorImagenes.aspx?DocumentoCodigo=" + NumeroDocumento + "&GrupoPadreCodigo=" + GrupoPadreCodigo + "&GrupoCodigo=" + GrupoCodigo + "&ImagenFolio=" + Folio;
            }
            else if (words[Ind].ToLower() == "png")
            {
                Hpl.ImageUrl = "~/AlfaNetImagen/iconos/icono_tif.jpg";
                Hpl.NavigateUrl = "VisorImagenes.aspx?DocumentoCodigo=" + NumeroDocumento + "&GrupoPadreCodigo=" + GrupoPadreCodigo + "&GrupoCodigo=" + GrupoCodigo + "&ImagenFolio=" + Folio;
            }
            else if (words[Ind].ToLower() == "bmp")
            {
                Hpl.ImageUrl = "~/AlfaNetImagen/iconos/icono_tif.jpg";
                Hpl.NavigateUrl = "VisorImagenes.aspx?DocumentoCodigo=" + NumeroDocumento + "&GrupoPadreCodigo=" + GrupoPadreCodigo + "&GrupoCodigo=" + GrupoCodigo + "&ImagenFolio=" + Folio;
            }
            else if (words[Ind].ToLower() == "gif")
            {

                Hpl.ImageUrl = "~/AlfaNetImagen/iconos/icono_tif.jpg";
                Hpl.NavigateUrl = "VisorImagenes.aspx?DocumentoCodigo=" + NumeroDocumento + "&GrupoPadreCodigo=" + GrupoPadreCodigo + "&GrupoCodigo=" + GrupoCodigo + "&ImagenFolio=" + Folio;
            }
            else if (words[Ind].ToLower() == "zip")
            {
